    ...
    public struct GameOverview: Codable, Equatable, Sendable {
        ...
        public struct GameTime: Codable, Equatable, Sendable {
            ...
            public func estimatedEndTime(_ gameDuration: String) -> Date? {
                // Parse the game duration string (format: "mm:ss")
                let components = gameDuration.split(separator: ":").compactMap { Int($0) }
                ...
                let elapsedSeconds = TimeInterval(minutes * 60 + seconds)

                // Set the maximum game duration to 20 minutes (in seconds)
                let maxGameDuration: TimeInterval = 20 * 60

                // Calculate remaining time
                let remainingSeconds = maxGameDuration - elapsedSeconds

                // Calculate the estimated end time by adding remaining time to current time
                let estimatedEndDate = Date().addingTimeInterval(remainingSeconds)
        ...
        public init(
            gameId: Int? = nil, homeTeam: TeamData, awayTeam: TeamData, homeGoals: Int,
            awayGoals: Int, state: GameState, gameUuid: String, time: GameTime
        ) {
            self.gameId = gameId
        ...
        public init(from decoder: any Decoder) throws {
            let container: KeyedDecodingContainer<GameData.GameOverview.CodingKeys> =
                try decoder.container(keyedBy: GameData.GameOverview.CodingKeys.self)
            ...
            } else if let gameIdString = try? container.decodeIfPresent(
                String.self, forKey: .gameId)
            {
            ...
            self.homeTeam = try container.decode(
                GameData.GameOverview.TeamData.self, forKey: .homeTeam)
            self.awayTeam = try container.decode(
                GameData.GameOverview.TeamData.self, forKey: .awayTeam)
